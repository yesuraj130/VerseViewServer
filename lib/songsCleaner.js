...
export function findSimilarDuplicates(db, minSimilarity = 70)
{
  ...
  {
    if (list.length > 30 || list.length < 2) continue; // Skip words common to too many songs or unique
    for (let i = 0; i < list.length; i++)
    {
      ...
      for (let j = i + 1; j < list.length; j++)
      {
        ...
        // Fast pre-filter: length ratio must be at least 0.60
        const minL = sA.cleanLen < sB.cleanLen ? sA.cleanLen : sB.cleanLen;
        const maxL = sA.cleanLen > sB.cleanLen ? sA.cleanLen : sB.cleanLen;
        if ((minL / maxL) < 0.60) continue;
