...
export function analyzeSongErrors(db)
{
  ...
  for (const r of rows)
  {
    ...
    slides.forEach((slide, sIdx) => {
      ...
      } else if (/[a-zA-Z];[a-zA-Z]/.test(slide.replace(/<[^>]+>/g, " "))) {
        baminiSlideIndices.push(sIdx + 1);
        if (!firstBaminiSnippet) {
          firstBaminiSnippet = (slide.replace(/<[^>]+>/g, " ").match(/[a-zA-Z];[a-zA-Z]/) || [])[0] || "";
        }
      }
    });
...
export function convertMixedSongToCleanUnicode(lyrics)
{
  ...
  const convertedSlides = slides.map(slide => {
    const hasUnicode = tamilUnicodeRegex.test(slide);
    const snip = detectBaminiInSlide(slide);
    const hasViramaPattern = /[a-zA-Z];[a-zA-Z]/.test(slide.replace(/<[^>]+>/g, " "));
    const hasKombuPattern = /N[fqrslzjtdgpmvwk]/.test(slide.replace(/<[^>]+>/g, " "));

    if (snip || (!hasUnicode && (hasViramaPattern || hasKombuPattern)))
    {
