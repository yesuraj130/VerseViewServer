...
export function detectBaminiInSlide(slideText)
{
  ...
  const text = slideText.replace(/<\/?(?:slide|br|b|i|u|p|span|font)\b[^>]*>/gi, ' ');
  
  // High-confidence Bamini patterns:
  // 0. Angle bracket followed by Bamini letter/virama: <e;, <rh, <th, <lh, <l;, <it, <d, <D, etc. (Bamini for Ee)
  const angleBamini = text.match(/<[a-zA-Z;0-9\\]/g) || [];
  if (angleBamini.length >= 1)
  {
    return angleBamini[0];
  }
...
export function analyzeSongErrors(db)
{
  ...
  for (const r of rows)
  {
    ...
    // Detailed slide-level scan for Bamini keystrokes
    const slides = lyrics.split(/<slide>/i);
    const baminiSlideIndices = [];
    let firstBaminiSnippet = "";
    slides.forEach((slide, sIdx) => {
      const snip = detectBaminiInSlide(slide);
      const cleanSlide = slide.replace(/<\/?(?:slide|br|b|i|u|p|span|font)\b[^>]*>/gi, " ");
      if (snip) {
        baminiSlideIndices.push(sIdx + 1);
        if (!firstBaminiSnippet) firstBaminiSnippet = snip;
      } else if (/[a-zA-Z];[a-zA-Z]/.test(cleanSlide) || /<[a-zA-Z;0-9\\]/.test(cleanSlide)) {
        baminiSlideIndices.push(sIdx + 1);
        if (!firstBaminiSnippet) {
          firstBaminiSnippet = (cleanSlide.match(/[a-zA-Z];[a-zA-Z]|<[a-zA-Z;0-9\\]+/) || [])[0] || "";
        }
      }
    });

    // Check: Song has Unicode font name (e.g. Baloo Thambi, Latha, etc. - NOT Tamil Bible/Bamini), but contains Bamini encoding in slides
    if (!isTB && !isKnownBamini && !isEng && (baminiSlideIndices.length > 0 || (isBaloo && hasBaminiInContent)))
    {
      const slideListStr = baminiSlideIndices.length > 0 ? baminiSlideIndices.join(", ") : "multiple";
      const snippet = firstBaminiSnippet || (hasBaminiInContent ? getBaminiSnippetInSong(lyrics, name, r.cat) : "") || "";
      let convertedPreview = "";
      try {
        if (snippet) convertedPreview = baminiToUnicode(snippet);
      } catch (e) {}

      issues.push({
        category: "baminiInUnicode",
        severity: "error",
        code: "BAMINI_IN_UNICODE",
        title: "Bamini Keystrokes in Unicode Font Song",
        description: `Song is declared with Unicode font "${font || "Baloo Thambi"}", but slide(s) [${slideListStr}] contain Bamini keystroke encoding. When projected, these slides show garbled ASCII letters instead of Tamil.`,
        baminiSlides: baminiSlideIndices,
        snippet,
        convertedSnippet: convertedPreview,
        suggestedFont: "Baloo Thambi",
        canAutoFix: true
      });
      counts.baminiInUnicode++;
    }
...
/**
 * Converts only the Bamini slides of a song into clean Unicode, leaving Unicode slides untouched.
 */
export function convertMixedSongToCleanUnicode(lyrics)
{
  if (!lyrics || typeof lyrics !== "string") return "";
  const slides = lyrics.split(/<slide>/i);

  const convertedSlides = slides.map(slide => {
    const cleanText = slide.replace(/<\/?(?:slide|br|b|i|u|p|span|font)\b[^>]*>/gi, " ");
    const snip = detectBaminiInSlide(slide);
    const hasViramaPattern = /[a-zA-Z];[a-zA-Z]/.test(cleanText);
    const hasKombuPattern = /N[fqrslzjtdgpmvwk]/.test(cleanText);
    const hasAngleBamini = /<[a-zA-Z;0-9\\]/.test(cleanText);

    if (snip || hasViramaPattern || hasKombuPattern || hasAngleBamini)
    {
      return baminiToUnicode(slide);
    }
    return slide;
  });

  return convertedSlides.join("<slide>");
}

/**
 * Converts a single song with mixed Bamini slides to 100% Unicode with Baloo Thambi font
 */
export function convertSingleMixedBamini(db, dataDir, songsDbFileName = "songs.db", songId, targetFont = "Baloo Thambi")
{
  silentBackupSongsDb(dataDir, songsDbFileName, db);
  ...
  const cleanLyrics = convertMixedSongToCleanUnicode(row.lyrics);
  const updateStmt = db.prepare("UPDATE sm SET lyrics = ?, font = ? WHERE id = ?");
  updateStmt.run(cleanLyrics, targetFont, songId);
  checkpointAndVacuum(db);
  ...
  return {
    ...
    songId,
    targetFont,
    originalLyrics: row.lyrics,
    convertedLyrics: cleanLyrics
  };
}

/**
 * Mass converts all songs detected with Bamini in Unicode content to clean Unicode with Baloo Thambi
 */
export function massConvertMixedBamini(db, dataDir, songsDbFileName = "songs.db", targetFont = "Baloo Thambi")
{
  const analysis = analyzeSongErrors(db);
  const mixedSongs = (analysis.songs || []).filter(s => s.issues.some(i => i.category === "baminiInUnicode"));

  if (mixedSongs.length === 0)
  {
    return { success: true, count: 0, message: "No songs with mixed Bamini in Unicode detected." };
  }
  ...
  try
  {
    for (const s of mixedSongs)
    {
      const cleanLyrics = convertMixedSongToCleanUnicode(s.rawLyrics);
      updateStmt.run(cleanLyrics, targetFont, s.id);
      count++;
      convertedIds.push(s.id);
    }
    db.exec("COMMIT;");
    checkpointAndVacuum(db);
  }
