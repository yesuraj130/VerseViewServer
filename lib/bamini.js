...
function convertLine(text) {
  ...
  // Pre-normalization for church typist quirks & database escaping
  s = s.replaceAll("\\\\;", "\\;");
  s = s.replaceAll("\\\\", "\\");
  s = s.replaceAll("!;", "];");
  s = s.replaceAll("![", "];");
  ...
  // Apply sskjames replacement rules
...
function baminiToUnicode(text) {
  ...
  // Tag Shielding: split by HTML / slide tags and newlines (<slide...>, <br...>, etc.)
  // and process lines/segments one by one so tags are never mutated
